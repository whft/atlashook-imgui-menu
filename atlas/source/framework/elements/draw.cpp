...
void c_draw::frame(ImDrawList* draw_list, const ImVec2& p_min, const ImVec2& p_max)
{
    const ImVec2 mn = ImFloor(p_min);
    const ImVec2 mx = ImFloor(p_max);
    draw->rect(draw_list, mn, mx, draw->get_clr(clr->window.stroke));
    draw->rect(GetForegroundDrawList(), mn - ImVec2(1, 1), mx + ImVec2(1, 1), draw->get_clr(clr->window.outline));
}

void c_draw::window_decorations()
{
    const ImVec2 pos = ImFloor(GetWindowPos());
    const ImVec2 size = ImFloor(GetWindowSize());
    ImDrawList* dl = GetWindowDrawList();

    draw->rect_filled(dl, pos, pos + size, draw->get_clr(clr->window.background_one));

    draw->frame(dl, pos, pos + size);
    {

        ImFont* title_font = var->font.fontnew ? var->font.fontnew : var->font.tahoma;
        const float ty = (var->window.titlebar - title_font->FontSize) * 0.5f + 2.f;
        const float tx = elements->content.window_padding.x;
        const char* name = GetCurrentWindow()->Name;

        if (name && strcmp(name, "atlashook") == 0)
        {
            const float fs = title_font->FontSize;
            const ImVec2 a_size = title_font->CalcTextSizeA(fs, FLT_MAX, 0.f, "atlas");
            draw->text_outline(GetWindowDrawList(), title_font, fs, GetWindowPos() + ImVec2(tx, ty), draw->get_clr(clr->widgets.text), "atlas");
            draw->text_outline(GetWindowDrawList(), title_font, fs, GetWindowPos() + ImVec2(tx + a_size.x, ty), draw->get_clr(clr->accent), "hook");
        }
        else
        {
            draw->text_outline(GetWindowDrawList(), title_font, title_font->FontSize, GetWindowPos() + ImVec2(tx, ty), draw->get_clr(clr->accent), name);
        }
    }
}