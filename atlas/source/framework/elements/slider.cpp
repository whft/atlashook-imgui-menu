...
bool slider_behavior_t(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        if (set_new_value && ((g.LastItemData.InFlags & ImGuiItemFlags_ReadOnly) || (flags & ImGuiSliderFlags_ReadOnly)))
            set_new_value = false;
...
{
    IM_ASSERT((flags == 1 || (flags & ImGuiSliderFlags_InvalidMask_) == 0) && "Invalid ImGuiSliderFlags flag!");
...
bool slider_scalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, bool wo_label, const char* format, ImGuiSliderFlags flags)
{
    ...
    const int data_size = DataTypeGetInfo(data_type)->Size;
    const char zero_bytes[16] = { 0 };
    const bool is_zero = data_size <= (int)sizeof(zero_bytes) && memcmp(p_data, zero_bytes, data_size) == 0;
    const ImU32 text_col = draw->get_clr(is_zero ? clr->widgets.text_inactive : clr->widgets.text);

    const ImVec2 inner_min = clickable.Min + ImVec2(2, 2);
    const ImVec2 inner_max = clickable.Max - ImVec2(2, 2);
    const ImVec4 accent_dim = { clr->accent.Value.x - 0.2f, clr->accent.Value.y - 0.2f, clr->accent.Value.z - 0.2f, 1.f };

    draw->fade_rect_filled(window->DrawList, inner_min, inner_max, draw->get_clr(clr->window.background_two), draw->get_clr(clr->window.background_one), fade_direction::vertically);
    draw->fade_rect_filled(window->DrawList, inner_min, grab_bb.Max - ImVec2(6, 0), draw->get_clr(clr->accent), draw->get_clr(accent_dim), fade_direction::vertically);
    ...
    if (!wo_label)
    {
        const ImVec2 row_min = rect.Min + ImVec2(1, -2);
        const ImVec2 row_max = ImVec2(rect.Max.x, clickable.Min.y - 2);
        draw->text_clipped_outline(window->DrawList, var->font.tahoma, row_min, row_max,                       text_col, label,     NULL,          NULL, ImVec2(0.f, 0.f));
        draw->text_clipped_outline(window->DrawList, var->font.tahoma, row_min, ImVec2(row_max.x - 1, row_max.y), text_col, value_buf, value_buf_end, NULL, ImVec2(1.f, 0.f));
    }
    else
    {
        draw->text_clipped_outline(window->DrawList, var->font.tahoma, clickable.Min - ImVec2(0, 1), clickable.Max - ImVec2(0, 1), text_col, value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
    }
